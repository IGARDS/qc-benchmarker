...
query_TIC <- chromatogram(query, aggregationFun = "max")

# query_TIC <- chromatogram(query, mz = c(600,610), aggregationFun = "max") # look in smaller windows for complex samples?

x <- as.numeric(query_TIC[1,1]@rtime)
y <- as.numeric(query_TIC[1,1]@intensity)
mean_diff_between_MS1_times <- mean(diff(x))

fy <- Re(fft(fft(y)*Conj(fft(y)), inverse = TRUE))

# plot(fy[1:100], type="l")
# plot(mean_diff_between_MS1_times*(1:100), diff((fy[1:101]))/diff(x[1:101]-x[1]), type="l")
# plot(mean_diff_between_MS1_times*(1:100), fy[1:100], type="l")

max_fy <- max(fy)
min_fy_w <- min(fy[1:100])
for(i in 1:100) {if(fy[i]<(min_fy_w+(max_fy-min_fy_w)/2)) break}
peak_width <- 2*i*mean_diff_between_MS1_times
cat('Mean peak width =', signif(peak_width, digits=4), "seconds")
...
```
\begin{center}Figure 2: Retention time coefficients for query compared with reference. The closer to the 45-degree diagonal the better. Deviation in basic and/or acidic residues suggests different pH.\end{center}
