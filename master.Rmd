...
```{r LC, echo=FALSE}
query <- readMSData(paste(QUERY, '.mzML', sep=''), msLevel = 1, mode=c("onDisk")) # default is not onDisk, but this seems to avoid memory issue and is faster
query_BPC <- chromatogram(query, aggregationFun = "max")  # this is 10x slower with onDisk object, but, memory returns to normal after
...
reference <- readMSData(paste(DATA_DIR, REFERENCE, '.mzML', sep=''), msLevel = 1, mode=c("onDisk"))
...
if((100*nrow(psms(reference_ids))/nrow(psms(query_ids))) < 50){
  shell('echo 3 > ms2.result')
} else if ((100*nrow(psms(reference_ids))/nrow(psms(query_ids))) < 80){
  shell('echo 2 > ms2.result')
} else {
  shell('echo 1 > ms2.result')
}
