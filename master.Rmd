...
system(paste('/local/tpp/bin/spectrast -sL', SPECTRAST_LIBRARY, ' -sD', FASTA_FILE,
             ' -sTAA -sA! -s_HOM4 -sR! -sEpep.xml ', OUTPUT_DIR, '/', QUERY, '.mzML', sep = ''))
system(paste('/local/tpp/bin/xinteract -N', sub('$', '.interact.pep.xml', QUERY), 
             ' -p0.95 -l7 -PPM -O -D', FASTA_FILE, ' ', OUTPUT_DIR, '/', QUERY, '.pep.xml', sep = ''))

			 
# Extract QC values from SpectraST search
# use already processed reference file which is DATA_DIR/REFERENCE.mzid (or interact.pep.xml or pep.xml)

system(paste('/local/tpp/bin/idconvert ', OUTPUT_DIR, '/', QUERY, '.interact.pep.xml', sep=''))

# mzID and all other files are in OUTPUT_DIR
reference_ids <- openIDfile(paste(DATA_DIR, REFERENCE, '.mzid', sep=''))
query_ids <- openIDfile(paste(OUTPUT_DIR, '/', QUERY, '.mzid', sep=''))
...
rmarkdown::render("/data/qc-benchmarker/sample.Rmd", output_dir = OUTPUT_DIR,output_file="sample.html",knit_root_dir=OUTPUT_DIR);
...
```{r LC, echo=FALSE}
query <- readMSData(paste(OUTPUT_DIR, '/', QUERY, '.mzML', sep=''), msLevel = 1) 
...
peptides <-c(); RTs <- c()
psms <- xmlToList(paste(OUTPUT_DIR, '/', QUERY, '.interact.pep.xml', sep=''))
...
rmarkdown::render("/data/qc-benchmarker/lc.Rmd", output_dir = OUTPUT_DIR,output_file="lc.html",knit_root_dir=OUTPUT_DIR)
...
rmarkdown::render("/data/qc-benchmarker/source.Rmd", output_dir = OUTPUT_DIR,output_file="source.html",knit_root_dir=OUTPUT_DIR)
...
# extract injection time statistics
query_header <- header(query_file)

# mean_reference_injectionTime <- mean(reference_header$injectionTime[reference_header$injectionTime<max(reference_header$injectionTime)])

mean_query_injectionTime <- mean(query_header$injectionTime[query_header$injectionTime<max(query_header$injectionTime)])

cat('Mean MS1 injection time: ', signif(mean_query_injectionTime, digits=2), ' sec.', sep='')
...
rmarkdown::render("/data/qc-benchmarker/ms1.Rmd", output_dir = OUTPUT_DIR,output_file="ms1.html",knit_root_dir=OUTPUT_DIR)
...
if((100*nrow(psms(query_ids))/nrow(psms(reference_ids))) < 50){
  system('echo 3 > ms2.result')
} else if ((100*nrow(psms(query_ids))/nrow(psms(reference_ids))) < 80){
...
rmarkdown::render("/data/qc-benchmarker/ms2.Rmd", output_dir = OUTPUT_DIR,output_file="ms2.html",knit_root_dir=OUTPUT_DIR)
...
system(paste('rm ',OUTPUT_DIR, '/', QUERY, '.mzML', sep=''))
