...
system(paste('wine idconvert ', OUTPUT_DIR,'/../',QUERY, '.interact.pep.xml', sep='')) 
...
rmarkdown::render("/data/qc-benchmarker/sample.Rmd")

```



5. LC; Estimate peak capacity. Calculate peak capacity as defined in QuiC or ID-free?

```{r LC, echo=FALSE}
query <- readMSData(paste(OUTPUT_DIR,'/../',QUERY, '.mzML', sep=''), msLevel = 1) # default is not onDisk, but this seems to avoid memory issue and is faster, but it makes the chromatogram() super slow
...
peptides <-c(); RTs <- c()
psms <- xmlToList(paste(tolower(OUTPUT_DIR,'/../',QUERY), '.interact.pep.xml', sep=''))
...
rmarkdown::render("/data/qc-benchmarker/lc.Rmd")
...
rmarkdown::render("/data/qc-benchmarker/source.Rmd")
...
rmarkdown::render("/data/qc-benchmarker/ms1.Rmd")
...
rmarkdown::render("/data/qc-benchmarker/ms2.Rmd")
